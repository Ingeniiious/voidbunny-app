...
export default function DecryptedText({
        ...
        if (sequential) {
          ...
          if (direction === 'reverse') {
            if (pointerRef.current < orderRef.current.length) {
              ...
              const newRevealed = new Set(prevRevealed);
              if (idxToRemove !== undefined) newRevealed.delete(idxToRemove);
              setDisplayText(shuffleText(text, newRevealed));
  ...
  /* eslint-disable react-hooks/set-state-in-effect -- vendor component (React Bits): syncs text/animateOn prop changes back into local state on mount and prop-swap. */
  useEffect(() => {
    if (animateOn === 'click') {
      encryptInstantly();
    } else {
